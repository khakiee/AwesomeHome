...
IPAddress hostIp;
IPAddress localIp;
char localIpAddress[] = "192.168.29.127";
uint8_t ret;
uint8_t localRet;
...
//      Serial.println(line);
      ...
      if(event_data_line>0) {
        String tmp_str="<li>";
        String event_data = line.substring(line.indexOf(tmp_str)+tmp_str.length(), event_data_line);
//        Serial.print("hour is "); 
...
void connectToServer() {
  ...
  if (client.connect(hostIp, 80)) {
    ...
    client.println("GET /wid/queryDFSRSS.jsp?zone=1120056000 HTTP/1.1\r"); 
    //위에 지정된 주소와 연결한다.
